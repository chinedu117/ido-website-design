...
export const NAIRA_CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NAIRA_CONTRACT_ADDRESS
...
export const getIDOContract = (provider: ethers.providers.Web3Provider) => {
  ...
  return new ethers.Contract(IDO_CONTRACT_ADDRESS, IDO_ABI.abi, provider.getSigner())
}

export const getNairaTokenContract = (provider: ethers.providers.Web3Provider) => {
  if (!NAIRA_CONTRACT_ADDRESS) {
    throw new Error("Naira token contract address not configured")
  }
  return new ethers.Contract(NAIRA_CONTRACT_ADDRESS, NAIRA_TOKEN_ABI.abi, provider.getSigner())
}
